...
def main():
    ...
    # ===================================================================
    # EXTENDED TESTS (Task 2 TDD - additional coverage)
    # ===================================================================

    # ===== Test 26: format_email_body contains footer =====
    try:
        body = format_email_body(MockPost())
        if "Generated by PUM Content Generator" not in body:
            errors.append("Test 26 (footer): 'Generated by PUM Content Generator' not in body")
    except Exception as e:
        errors.append(f"Test 26 (footer): {type(e).__name__}: {e}")

    # ===== Test 27: send_post_email lists all missing vars when none set =====
    saved = clear_email_env()
    try:
        try:
            send_post_email(MockPost(), "any_path.png")
            errors.append("Test 27 (all missing): No ValueError raised")
        except ValueError as e:
            error_msg = str(e)
            if "GMAIL_ADDRESS" not in error_msg:
                errors.append(f"Test 27 (all missing): 'GMAIL_ADDRESS' not in error: {e}")
            if "GMAIL_APP_PASSWORD" not in error_msg:
                errors.append(f"Test 27 (all missing): 'GMAIL_APP_PASSWORD' not in error: {e}")
            if "RECIPIENT_EMAIL" not in error_msg:
                errors.append(f"Test 27 (all missing): 'RECIPIENT_EMAIL' not in error: {e}")
        except Exception as e:
            errors.append(f"Test 27 (all missing): Wrong exception {type(e).__name__}: {e}")
    finally:
        restore_env(saved)
